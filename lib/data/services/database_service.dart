...
class DatabaseService {
  ...
  Future<void> _seedSampleData() async {
    ...
    final mojito = RecipesCompanion.insert(
      ...
      about: const Value('A traditional Cuban highball with a refreshing mix of lime, mint, and rum.'),
    ...
    // Add tags
    final mintTagId = await database.into(database.tags).insert(TagsCompanion.insert(value: 'Mint'));
    final rumTagId = await database.into(database.tags).insert(TagsCompanion.insert(value: 'Rum'));
    final summerTagId = await database.into(database.tags).insert(TagsCompanion.insert(value: 'Summer'));

    // Associate tags with recipe
    await database.into(database.recipeTags).insert(RecipeTagsCompanion.insert(recipeId: mojitoId, tagId: mintTagId));
    await database.into(database.recipeTags).insert(RecipeTagsCompanion.insert(recipeId: mojitoId, tagId: rumTagId));
    await database.into(database.recipeTags).insert(RecipeTagsCompanion.insert(recipeId: mojitoId, tagId: summerTagId));

    // Add instructions
    await database.into(database.instructions).insert(
    ...
    await database.into(database.instructions).insert(
      InstructionsCompanion.insert(
        recipeId: mojitoId,
        value: 'Add a splash of soda water and fill the glass with cracked ice.',
        step: 2
      )
    );
    await database.into(database.instructions).insert(
    ...
    final oldFashioned = RecipesCompanion.insert(
      ...
      imagePath: const Value(null),
      about: const Value('A sophisticated whiskey cocktail that\'s stood the test of time.'),
    ...
    final oldFashionedId = await database.into(database.recipes).insert(oldFashioned);

    // Add tags
    final whiskeyTagId = await database.into(database.tags).insert(TagsCompanion.insert(value: 'Whiskey'));
    final classicTagId = await database.into(database.tags).insert(TagsCompanion.insert(value: 'Classic'));

    // Associate tags with recipe
    await database.into(database.recipeTags).insert(RecipeTagsCompanion.insert(recipeId: oldFashionedId, tagId: whiskeyTagId));
    await database.into(database.recipeTags).insert(RecipeTagsCompanion.insert(recipeId: oldFashionedId, tagId: classicTagId));

    // Add instructions for Old Fashioned
    await database.into(database.instructions).insert(
      InstructionsCompanion.insert(
        recipeId: oldFashionedId,
        value: 'Place sugar cube in old-fashioned glass and saturate with bitters, add a dash of water.',
        step: 1
      )
    );
    await database.into(database.instructions).insert(
